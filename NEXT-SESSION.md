...
(read them in `handoff.py amendments` history / WORK-LOG). PROGRESS (automatic): start each commit subject with the item's tag words, e.g. "UI4 item 3: …" — the progress page
counts an item done when a commit subject contains its tag (hyphens = spaces). No ticking needed.

## Checklist
- [ ] [UI4-item-0] BUG FIRST (Fred + advisor reproduced headless on live main): in the SHAPE LATTICE, the Select icon drag shows the
...
- [ ] [UI4-item-0b] BUG (Fred, live): CLEAR the canvas (toolbar Clear) then press Regenerate -> nothing is generated. Reproduce on both
...
- [ ] [UI4-item-0c] BUG (Fred): layer holds a SHAPE lattice -> switch to the BOX Lattice tool -> its Regenerate rebuilds the SHAPE.
...
- [x] [UI4-item-1] Pinned-action style (AMEND 4b): ONE shared sticky style for the main sidebar "Generate New Seed" and both lattice
...
- [x] [UI4-item-2] Screenshots of all three scrolled mid-list, desktop + ~1024px (iPad) — viewed before ticking.
- [ ] [UI4-item-3] Per-piece overrides (AMEND 3): selected rail/tie/node/contour piece shows COLOUR + WIDTH with an override; stored as
      declared data (data-override-color / data-override-width, one schema), rendered live.
- [ ] [UI4-item-4] Regenerate clears overrides (no warning — Fred); Undo restores them.
- [ ] [UI4-item-5] Tests for 1, 3, 4; full suite green; WORK-LOG entry.
- [x] [UI4-item-6] BUG (Fred, live): a layer added from the MAIN sidebar (Vector Stamping → Layers +, e.g. "Layer 2") does NOT exist
...
- [x] [UI4-item-7] The side-column Layers row truncates the name ("Lay…") next to its eye/3D/palette buttons — let the name use the
