...
REFRESH_COMMAND_ID   = 'fusionHybridRefreshCommand'
REFRESH_COMMAND_NAME = 'Refresh Add-in'
REFRESH_COMMAND_TOOLTIP = 'Refreshes the add-in by stopping and starting it again'
    ...
    try:
        ...
        if ws:
            ...
            if tab:
                ...
                if panel:
                    for cid in [COMMAND_ID, REFRESH_COMMAND_ID]:
                        cntrl = panel.controls.itemById(cid)
                        if cntrl: cntrl.deleteMe()

                    if panel.controls.count == 0:
                        panel.deleteMe()

        # 4. Remove command definitions
        for cid in [COMMAND_ID, REFRESH_COMMAND_ID]:
