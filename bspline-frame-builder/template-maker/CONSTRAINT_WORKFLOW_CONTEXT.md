...
- **CoincidentConstraint: dedicated-module extraction.** The generic `CONSTRAINT_TARGET_PROPS_BY_TYPE` slot dispatch doesn't fit Coincident. Two distinct crash modes were verified via `[probe-*]` / `[foc-*]` / `[gate-*]` diagnostic log traces:
  - Direct slot probe: reading `.point` or `.entity` on a CoincidentConstraint proxy either raises a Python-catchable `RuntimeError: 1 : vector too long` AND then delayed-native-AVs Fusion ~4 s later (log timestamps 20:40:57, 20:54:05), or native-AVs immediately with no Python-visible exception (21:02:06). Slot touch is never safe.
  - Indirect probe via `offset_hint.find_owning_offset_constraint`: `_safe_getattr(curve, 'parentSketch')` on a CoincidentConstraint is also a delayed-native-AV site (20:58:47 — last durable line before crash was exactly that probe).
  
  Current mitigation (this session): `'CoincidentConstraint': ()` empty-tuple entry in `CONSTRAINT_TARGET_PROPS_BY_TYPE`, plus a type guard in `template_payload_builder._expand_offset_picks` that skips the offset reverse-lookup for entities whose type ends in `Constraint` or `Dimension`. Net effect: Fusion stays alive, Coincident is treated as "untagged/skipped" in the palette, no code output.
  
  Proper fix: dedicated `coincident_hint.py`, modelled on `offset_hint.py`. The module would own Coincident end-to-end — identity reverse-walk (from `sketch.sketchPoints` and each point's outgoing constraints, NOT from the Coincident's own slots), `build_coincident_step()` emitter, and an `is_coincident_constraint()` classifier wired into `_expand_offset_picks` as a sibling pass. See §1.2 — Coincident's data model (point + entity, with point-on-point as a valid variant that doesn't cleanly fit the `.point`/`.entity` slot shape) was the underlying mismatch all along; the slot-dispatch table was never the right abstraction for it.

- **Projection recognition in the main payload flow.** `detect_projections.py` (349-line dedicated module, same shape as `offset_hint.py`) exists and is fully functional but is ONLY wired into `template_bridge.py` — the palette-side classify-selection entry point. The main Template Maker payload/hint pipeline (`template_payload_builder.build_payload_items` → `ownership_gate.is_framebuilder_owned` → emitter) does NOT call it. Result: projected geometry falls through to the "untagged/skipped" bucket in the same flow Coincident now does.
  
  The architectural shape this reveals: `CONSTRAINT_TARGET_PROPS_BY_TYPE` is implicitly the table for "fixed-shape, 1-or-2-slot, type-clean constraints." A second, currently-unnamed category exists: **dedicated-dispatch entities** — OffsetConstraint (extracted and fully integrated via `offset_hint.py`), Projections (extracted but palette-only), CoincidentConstraint (pending extraction). All three share: identity flows from external references (parent curves / source sketch / participant points), they can't carry a direct `FrameBuilder.ID` attribute, and the generic slot dispatch is structurally wrong for them.
  
  Incremental follow-up order: (1) name the "dedicated-dispatch" category in code — a registry that both `_expand_offset_picks` and the gate consult — so empty-tuple entries in `CONSTRAINT_TARGET_PROPS_BY_TYPE` become explicit cross-references rather than look like missing data; (2) wire `detect_projections` into `build_payload_items` as a sibling pre-pass to `_expand_offset_picks`; (3) build `coincident_hint.py` as the third dedicated-dispatch module.

- **Construction geometry not recognized.** Picks of construction SketchLines / SketchPoints fall through the ownership gate identically to untagged user-drawn geometry (log 21:21:46 / 21:21:52 / 21:21:59 / 21:22:03 / 21:22:07 — all SketchLine, all `[gate-exit] fallthrough type=adsk::fusion::SketchLine -> False`). Construction geometry is untagged by design — it's scaffolding, not seeds, and nothing in the runtime ever stamps it with `FrameBuilder.ID`. Architecturally this is a fourth sibling to the dedicated-dispatch category: the signal (`ent.isConstruction` — SketchLine and SketchPoint both expose it) is specific enough that it deserves its own branch in the gate rather than being silently lumped in with "untagged user geometry." Deferred: user flagged as "not serious now, take note while we debug coincident and proj." Follow-up after the projection strand closes. Instrumentation shape when we get to it: `[gate-step] branch: construction-check` → `[gate-const] isConstruction=<bool>` → either a dedicated label path or explicit unowned-with-reason=construction.

### Projection-path instrumentation added (this session)

The `[proj-*]` probe family now mirrors `[foc-*]` / `[gate-*]` for the projection path. Added in `detect_projections.py` (on `_is_projected`, `classify_selection`, `detect_projections`) plus `[proj-kind]` marker at the bridge call site in `template_bridge.py`. Purpose: close the visibility gap where a projection SketchLine pick logs "0 owned, 1 unowned" from the seed-path side but nothing from the projection classifier's side — so it's impossible to tell whether `_is_projected` returned False (miss-classified as 'seeds'), True (correctly routed but still shows the unowned banner — palette-side branch), or raised on the `isReference` read. Pre-instrumentation log at 21:13:07 / 21:21:46 / 21:21:52 / 21:21:59 / 21:22:03 / 21:22:07 showed only the seed-path side. A fresh capture of the same SketchLine pick under the new instrumentation is the next required data point before committing to any projection-side fix.
