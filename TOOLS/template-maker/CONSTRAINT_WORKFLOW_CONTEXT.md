...
- **CircularPatternConstraint.** Not in the subtype dispatch table. Exposes target collections rather than single slots, so `target_props_for` returns `()` and the emitter falls through to `/* targets */`. Would need either an entry in `CONSTRAINT_TARGET_PROPS_BY_TYPE` (if it fits the generic shape) or a dedicated module (like `offset_hint.py`) if its runtime representation differs. OffsetConstraint took the dedicated-module route this session.
